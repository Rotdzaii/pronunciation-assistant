...
OUTPUT_NAMES = (
    "r5_2b_tc1_execution_preflight.json",
    "r5_2b_tc1_population_accounting.json",
    "r5_2b_tc1_candidate_audit.json",
    "r5_2b_tc1_train_scores.jsonl",
    "r5_2b_tc1_continuous_metrics.json",
    "r5_2b_tc1_loso_thresholds.json",
    "r5_2b_tc1_oof_predictions.jsonl",
    "r5_2b_tc1_binary_metrics.json",
    "r5_2b_tc1_relation_cohort_metrics.json",
    "r5_2b_tc1_event_metrics.json",
    "r5_2b_tc1_gate_results.json",
    "r5_2b_tc1_execution_protocol_audit.json",
    "r5_2b_tc1_final_status.json",
    "r5_2b_tc1_compute_report.json",
    "R5_2B_TC1_TRAIN_DEVELOPMENT_RESULT.md",
    "R5_2B_TC1_EXECUTION_MANIFEST.json",
)
...
def verify_identities() -> dict[str, Any]:
    ...
    if mismatch:
        raise RuntimeError(f"R5_2B_TC1_EXECUTION_BLOCKED_IDENTITY: {mismatch}")
    manifest_paths = {
    ...
    if any(audit["status"] != "PASS" for audit in manifest_audits.values()):
        raise RuntimeError(f"R5_2B_TC1_EXECUTION_BLOCKED_IDENTITY: {manifest_audits}")
    checkpoint = torch.load(CHECKPOINT_PATH, map_location="cpu", weights_only=False)
    if int(checkpoint["blank_index"]) != 40 or tuple(checkpoint["vocabulary"]) != PHONE_VOCAB:
        raise RuntimeError("R5_2B_TC1_EXECUTION_BLOCKED_IDENTITY: checkpoint vocabulary mismatch")
    del checkpoint
    return {
...
def validate_population(words: list[dict[str, Any]], accounting: dict[str, Any]) -> None:
    ...
    if actual != EXPECTED_COUNTS or len(words) != EXPECTED_COUNTS["words"]:
        accounting["status"] = "R5_2B_TC1_EXECUTION_BLOCKED_ROW_ACCOUNTING"
...
def materialize_and_infer(words: list[dict[str, Any]], device: torch.device) -> tuple[list[torch.Tensor], list[torch.Tensor], dict[str, Any]]:
        ...
        for start in range(0, len(ordered), 8):
            ...
            for position, index in enumerate(indexes):
                ...
                if steps != expected_lengths[index]:
                    raise RuntimeError("R5_2B_TC1_EXECUTION_TECHNICAL_FAILURE_IMPLEMENTATION: output length mismatch")
    ...
    if any(value is None for value in raw_logits) or any(value is None for value in log_probs):
        raise RuntimeError("R5_2B_TC1_EXECUTION_TECHNICAL_FAILURE_IMPLEMENTATION: incomplete inference")
    return (
...
def candidate_audit(words: list[dict[str, Any]], logits: list[torch.Tensor]) -> dict[str, Any]:
    ...
    if int(sum(totals.values())) != expected_total:
        raise RuntimeError(
            f"R5_2B_TC1_EXECUTION_TECHNICAL_FAILURE_IMPLEMENTATION: "
            f"candidate total {sum(totals.values())}!={expected_total}"
        )
    return {
        "status": "PASS" if not keep_impossible_details else "R5_2B_TC1_EXECUTION_BLOCKED_KEEP_ALIGNABILITY",
...
def batch_ctc_scores(log_probs: list[torch.Tensor], items: list[tuple[int, tuple[int, ...]]], device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    if any(not target for _word_index, target in items):
        raise RuntimeError("R5_2B_TC1_EXECUTION_TECHNICAL_FAILURE_IMPLEMENTATION: empty target entered nonempty CTCLoss batch")
    ...
    for column, (word_index, target) in enumerate(items):
        ...
        if not scorer.is_alignable(target, evidence.shape[0]):
            raise RuntimeError("R5_2B_TC1_EXECUTION_TECHNICAL_FAILURE_IMPLEMENTATION: impossible target entered CTCLoss batch")
    ...
    if not np.isfinite(nll).all():
        raise RuntimeError("R5_2B_TC1_EXECUTION_TECHNICAL_FAILURE_NUMERICAL: alignable non-finite loss")
    raw = -nll; target = raw / np.asarray(target_lengths, dtype=np.float64)
    if not np.isfinite(target).all():
        raise RuntimeError("R5_2B_TC1_EXECUTION_TECHNICAL_FAILURE_NUMERICAL: derived non-finite score")
    return raw, target
...
def verify_batch_adapter(device: torch.device) -> dict[str, Any]:
    ...
    if float(raw[0]) != frozen.raw_score or float(target[0]) != frozen.target_score:
        raise RuntimeError(f"R5_2B_TC1_EXECUTION_TECHNICAL_FAILURE_IMPLEMENTATION: batch adapter mismatch {raw[0]}/{target[0]} vs {frozen.raw_score}/{frozen.target_score}")
...
def score_train(
    ...
    for word_index, word in enumerate(words):
        ...
        if not keep.hypothesis.alignable or not math.isfinite(keep.hypothesis.target_score):
            raise RuntimeError("R5_2B_TC1_EXECUTION_BLOCKED_KEEP_ALIGNABILITY")
    ...
    if processed_nonempty != expected_nonempty:
        raise RuntimeError(f"R5_2B_TC1_EXECUTION_TECHNICAL_FAILURE_IMPLEMENTATION: {processed_nonempty}!={expected_nonempty}")
    return output, {
...
def threshold_selection(scores: np.ndarray, truth: np.ndarray, correct_only: np.ndarray) -> tuple[float, dict[str, Any]]:
    ...
    if best_record is None or best_metrics is None:
        raise RuntimeError("R5_2B_TC1_EXECUTION_TECHNICAL_FAILURE_IMPLEMENTATION: threshold selection empty")
...
def build_report(accounting: dict[str, Any], audit: dict[str, Any], continuous: dict[str, Any], folds: list[dict[str, Any]], binary: dict[str, Any], cohorts: dict[str, Any], events: dict[str, Any], gates: dict[str, Any], status: str) -> str:
    ...
    lines = [
        "# R5-2B-TC1 Corrected Frozen TRAIN Development Result", "", f"Status: `{status}`", "",
        "## Population", "",
...
def execute() -> None:
    ...
    preflight = {
        "status": "PASS",
        "execution_label": "TECHNICAL CORRECTION OF THE PRE-METRIC R5-2B EXECUTION",
    ...
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_execution_preflight.json", preflight)
    ...
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_population_accounting.json", accounting)
    ...
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_candidate_audit.json", audit)
    ...
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_continuous_metrics.json", continuous)

    predictions, folds = run_loso(words, scores)
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_loso_thresholds.json", {
    ...
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_binary_metrics.json", binary)
    ...
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_relation_cohort_metrics.json", relation_cohorts)
    events = event_metrics(words, predictions, scored)
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_event_metrics.json", events)
    ...
    write_jsonl(EXPERIMENT_DIR / "r5_2b_tc1_train_scores.jsonl", score_rows)
    write_jsonl(EXPERIMENT_DIR / "r5_2b_tc1_oof_predictions.jsonl", oof_rows)
    ...
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_gate_results.json", gates)
    ...
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_execution_protocol_audit.json", protocol)
    final = {
    ...
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_final_status.json", final)
    compute = {
    ...
    write_json(EXPERIMENT_DIR / "r5_2b_tc1_compute_report.json", compute)
    report = build_report(accounting, audit, continuous, folds, binary, relation_cohorts, events, gates, status)
    (EXPERIMENT_DIR / "R5_2B_TC1_TRAIN_DEVELOPMENT_RESULT.md").write_text(report, encoding="utf-8")

    artifact_names = ["r5_2b_train_execution_driver_tc1.py"] + [
        name for name in OUTPUT_NAMES if name != "R5_2B_TC1_EXECUTION_MANIFEST.json"
    ]
    entries = [
    ...
    manifest = {
        "manifest_type": "additive corrected R5-2B-TC1 frozen TRAIN execution manifest", "self_excluded": True,
    ...
    write_json(EXPERIMENT_DIR / "R5_2B_TC1_EXECUTION_MANIFEST.json", manifest)
    print(json.dumps({
        "status": status, "gates_passed": passed_count, "manifest_sha256": sha256(EXPERIMENT_DIR / "R5_2B_TC1_EXECUTION_MANIFEST.json"),
