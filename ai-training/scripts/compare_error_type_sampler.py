...
RUNS = [
    {
        ...
        "note": "Original CNN baseline.",
    },
    {
        "name": "v2",
        "path": EVALUATION_DIR / "v2_error_type_eval_metrics.json",
        "note": "V2 with imbalance handling, focal loss/sampler/augmentation.",
    },
    {
        "name": "context_v3",
        "path": EVALUATION_DIR / "context_v3_error_type_eval_metrics.json",
        "note": "Context-window V3 experiment.",
    },
    {
        "name": "sampler_only",
        "path": EVALUATION_DIR / "sampler_error_type_eval_metrics.json",
        "note": "Clean dataset v2 with WeightedRandomSampler only.",
    },
]

OUTPUT_CSV = EVALUATION_DIR / "sampler_error_type_comparison.csv"
...
def get_split_metrics(data: dict, split_name: str):
    ...
    Older baseline/V2 format:
      data["splits"]["val"]["metrics"]
      data["splits"]["test"]["metrics"]
    """
...
def get_class_f1(split_metrics: dict, label: str):
    ...
    # Some older scripts may store class metrics under another key.
    per_class_metrics = split_metrics.get("per_class_metrics")

    if isinstance(per_class_metrics, dict):
        class_metrics = per_class_metrics.get(label)
...
def main():
    ...
    ranked = sorted(
        rows,
        key=lambda row: safe_float(row["test_macro_f1"]),
        reverse=True,
    )

    print()
    print("Ranking by test_macro_f1:")

    for index, row in enumerate(ranked, start=1):
        print(f"{index}. {row['run']} - test_macro_f1={row['test_macro_f1']}")
