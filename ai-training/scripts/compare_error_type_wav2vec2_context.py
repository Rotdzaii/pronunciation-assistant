...
OUTPUT_CSV = EVALUATION_DIR / "wav2vec2_context_full_comparison.csv"
...
RUNS = [
    ...
    ("wav2vec2_encoder", EVALUATION_DIR / "wav2vec2_encoder_error_type_eval_metrics.json", "Frozen Wav2Vec2 original segment."),
]
...
def get_metric(metrics: dict, key: str):
    aliases = {"accuracy": ["accuracy", "acc"], "macro_f1": ["macro_f1", "macro_f1_score"], "weighted_f1": ["weighted_f1", "weighted_f1_score"]}
...
def main():
    ...
    if context_data is not None:
        for crop_mode, run in context_data.get("runs", {}).items():
            run_name = f"wav2vec2_context_{crop_mode}" if crop_mode == "original_segment" else f"wav2vec2_{crop_mode}"
            rows.append(extract_row(run_name, run, "Frozen Wav2Vec2 context-window encoder."))
