...
CHECKPOINT_PATH = Path("ai-training/models/l2_arctic_addition_binary_cnn.pt")
...
SOURCE_LABELS = ["addition", "deletion", "substitution"]
LABEL_ORDER = ["addition", "non_addition"]
...
def add_binary_label(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["binary_error_type"] = np.where(df["error_type"] == "addition", "addition", "non_addition")
    return df
...
class ErrorSegmentDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        label = self.label_to_index[row["binary_error_type"]]
        ...
        rows.append(
            {
                ...
                "true_binary_error_type": true_name,
                "predicted_binary_error_type": predicted_name,
                "confidence": float(max(probabilities)),
                "is_correct": true_name == predicted_name,
                "prob_addition": float(probabilities[label_to_index["addition"]]),
                "prob_non_addition": float(probabilities[label_to_index["non_addition"]]),
            }
        )
...
def main():
    ...
    df = add_binary_label(df[df["error_type"].isin(SOURCE_LABELS)].copy())
    ...
    print(val_df["binary_error_type"].value_counts())
    print()
    print("Test distribution:")
    print(test_df["binary_error_type"].value_counts())
    ...
    output_metrics = {
        "model": "l2_arctic_addition_binary_cnn",
    ...
    metrics_path = OUTPUT_DIR / "addition_binary_eval_metrics.json"
    confusion_matrix_path = OUTPUT_DIR / "addition_binary_confusion_matrix.csv"
    per_class_path = OUTPUT_DIR / "addition_binary_per_class_metrics.csv"
    misclassified_path = OUTPUT_DIR / "addition_binary_misclassified_examples.csv"
