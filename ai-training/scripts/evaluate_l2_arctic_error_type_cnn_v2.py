...
METRICS_JSON = EVALUATION_DIR / "v2_error_type_eval_metrics.json"
CONFUSION_MATRIX_CSV = EVALUATION_DIR / "v2_error_type_confusion_matrix.csv"
PER_CLASS_CSV = EVALUATION_DIR / "v2_error_type_per_class_metrics.csv"
PER_SPEAKER_CSV = EVALUATION_DIR / "v2_error_type_per_speaker_metrics.csv"
MISCLASSIFIED_CSV = EVALUATION_DIR / "v2_error_type_misclassified_examples.csv"
CONFUSION_MATRIX_PNG = EVALUATION_DIR / "v2_error_type_confusion_matrix.png"
...
def maybe_write_confusion_matrix_png(results: dict[str, dict], label_to_error: dict[int, str]) -> bool:
    ...
    ax.set_title("V2 Validation Confusion Matrix")
...
def print_split_summary(split: str, result: dict):
    ...
    for example in result["misclassified_examples"][:10]:
        label = example["label"].encode(
            sys.stdout.encoding or "utf-8",
            errors="backslashreplace",
        ).decode(sys.stdout.encoding or "utf-8")
        print(
            "- "
            f"{example['speaker_id']} {example['utterance_id']} "
            f"{example['actual_error_type']} -> {example['predicted_error_type']} "
            f"{label}"
        )
...
def main():
    ...
    results = {
        ...
        "task": checkpoint.get("task", "l2_arctic_phone_error_type_classification_v2"),
