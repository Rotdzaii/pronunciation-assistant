...
DEFAULT_METADATA_CSV = Path("ai-training/datasets/l2-arctic/metadata/all_speakers_phone_error_classification_v2.csv")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run all-speaker CNN Attention phone error type inference.")
    parser.add_argument("--row-index", type=int, required=True, help="Metadata CSV row index to infer.")
    parser.add_argument(
        "--metadata-csv",
        type=Path,
        default=DEFAULT_METADATA_CSV,
        help="Metadata CSV to read. Defaults to all-speaker L2-ARCTIC v2 metadata.",
    )
    return parser.parse_args()


def read_rows(metadata_csv: Path) -> list[dict[str, str]]:
...
def row_from_index(metadata_csv: Path, row_index: int) -> dict[str, str]:
...
def main() -> None:
    args = parse_args()
    row = row_from_index(args.metadata_csv, args.row_index)
