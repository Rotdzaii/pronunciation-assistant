...
def main():
    ...
    parser.add_argument("--crop-mode", choices=list(CROP_MODES), required=True)
    args = parser.parse_args()
    path = checkpoint_path(args.crop_mode)
    if not path.exists():
    ...
    model = Wav2Vec2EncoderClassifier(checkpoint.get("model_name", MODEL_NAME), len(label_to_index), checkpoint.get("freeze_encoder", True)).to(device)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()
    feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(checkpoint.get("model_name", MODEL_NAME))
    segment, crop_start, crop_end = crop_segment(row, CROP_MODES[args.crop_mode])
    ...
    print("crop_mode:", args.crop_mode)
