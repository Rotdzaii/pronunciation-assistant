...
MODEL_OUTPUT = Path("ai-training/models/l2_arctic_error_type_cnn_sampler.pt")
...
def print_metrics(prefix: str, metrics: dict):
    ...
    for label_name in LABEL_ORDER:
        class_f1 = metrics["per_class"][label_name]["f1"]
        print(f"  {prefix}_f1_{label_name}={class_f1:.4f}")
...
def validate_audio_files(df: pd.DataFrame):
    ...
    if missing:
        ...
        raise FileNotFoundError(
            f"Missing audio files: {len(missing)}\n{preview}"
        )
...
def main():
    ...
    # Important:
    # Use normal CrossEntropyLoss here.
    # Class imbalance is handled by WeightedRandomSampler only.
            ...
            torch.save(
                {
                    ...
                    "task": "l2_arctic_phone_error_type_sampler_only",
